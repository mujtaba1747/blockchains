...
func networkWithCommitObjects(t *testing.T, n int) (*network.Network, []*types.Commit) {
	...
	for i := 0; i < n; i++ {
		state.CommitList = append(state.CommitList, &types.Commit{Creator: "ANY", Index: strconv.Itoa(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Commit)
			}
		})
	}
}
...
func TestListCommit(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Commit[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Commit)
	})
}
