...
func TestWhoisQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetWhoisRequest{Index: msgs[0].Index},
			response: &types.QueryGetWhoisResponse{Whois: &msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetWhoisRequest{Index: msgs[1].Index},
			response: &types.QueryGetWhoisResponse{Whois: &msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetWhoisRequest{Index: "missing"},
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			...
			for j := i; j < len(msgs) && j < i+step; j++ {
				assert.Equal(t, &msgs[j], resp.Whois[j-i])
			}
		}
	})
